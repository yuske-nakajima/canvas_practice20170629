module.exports = {
    ...
    "rules": {
        ...
        "no-reserved-keys": [2],// オブジェクトのキーに予約語を使用することを許可しない
        ...
        "dot-location": [2],// 一貫性のない改行前、改行後のドットを許可しない
        ...
        "no-empty-label": [2],//繰り返し処理やswitch文以外に対してのラベルの使用を許可しない
        ...
        "no-unused-expressions": [2],// 使用することのない式を許可しない
        ...
        "vars-on-top": [2],// 関数スコープの最上部で変数宣言をまとめないことを許可しない
        "wrap-iife": [2],// 即時関数を括弧で囲わないことを許可しない
        "yoda": [2],// 条件の値が左辺にくる条件式(ヨーダ)を許可しない

        // Strict Mode
        "strict": [2, "global"],// Strictモードのモード指定
        ...
        // Stylistic Issues
        "array-bracket-spacing": [2, "never"],// []の中でスペースを許可する(always)/しない(never)
        "brace-style": [2],// 特定の波括弧のスタイルのみ許可する
        ...
        "func-style": [2, "expression"],// 関数宣言か関数式かスタイルを統一することを要求する
        "indent": [2, "tab"],// インデントをタブかスペースn個かを指定する
        "key-spacing": [2, {// オブジェクトリテラルのコロンの前後にスペースの有/無を許可しない
            "beforeColon": false,
            "afterColon": true
        }],
        "lines-around-comment": [2, {// コメントの前後に空白行を入れることを要求する
        ...
        "no-ternary": [2],// 三項演算子の使用を許可しない
        "no-trailing-spaces": [2],// 行末の不要なスペースを許可しない
        "no-underscore-dangle": [2],// 識別子の先頭・末尾の_を許可しない
        ...
        "operator-assignment": [2],// 演算子でショートハンドに置き換え可能な場合に置き換えることを要求する
        "operator-linebreak": [2, "before"],// 演算子を改行前/後につけるスタイルに統一する
        "padded-blocks": [2, "always"],// ブロックの中で最初と最後に空白行を入れないことを許可しない
        ...
        "space-after-keywords": [2],// ifやforのようなキーワードの直後にスペースを入れないことを許可しない
        ...
        "space-return-throw-case": [2],//return、throw、caseの直後にスペースを入れないことを許可しない
        "space-unary-ops": [2],// 単項演算子の前後にスペース有無を指定する
        "spaced-comment": [2],// コメントの直前にスペースorタブの有無を指定する
    ...
    // "ecmaFeatures": {
        ...
        // "binaryLiterals": true,// 2進数リテラルを有効にする
        // "blockBindings": true,// letとconstを有効にする
        // "classes": true,// classを有効にする
        // "defaultParams": true,// デフォルトパラメータを有効にする
        // "destructuring": true,// 構造化代入を有効にする
        // "forOf": true,// for-ofを有効にする
        // "generators": true,// generatorsを有効にする
        // "modules": false,// moduleを有効にして、globalのstrict modeにする
        // "objectLiteralComputedProperties": true,// computedプロパティを有効にする
        // "objectLiteralDuplicateProperties": true,// strict modeでのプロパティの重複を有効にする
        // "objectLiteralShorthandMethods": true,// ショートハンドメソッドを有効にする
        // "objectLiteralShorthandProperties": true,// ショートハンドプロパティを有効にする
        // "octalLiterals": true,// 8進数リテラルを有効にする
        // "regexUFlag": true,// 正規表現でのuフラグを有効にする
        // "regexYFlag": true,// 正規表現でのyフラグを有効にする
        // "restParams": true,// 可変長引数を有効にする
        // "spread": true,// spread operatorを有効にする
        // "superInFunctions": true,// 関数内のsuperを有効にする
        // "templateStrings": true,// テンプレート文字列を有効にする
        // "unicodeCodePointEscapes": true,// code point escapes を有効にする
        // "globalReturn": true,// グローバルスコープでのreturnを有効にする
        // "jsx": true// JSXを有効にする
    // }
};